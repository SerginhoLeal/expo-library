...
export const ModalScreen: React.FC<StateProps> = ({ data, state, screen, onBackButtonPress }: StateProps) => {
  ...
  React.useEffect(() => {
    ...
    scrollX.addListener(({ value }) => {
      const val = Math.round(value / WIDTH_FOR_IMAGE);
      setIndex(val)
    });
  ...
  return (
    <Styles.Container statusBarTranslucent animationIn="fadeIn" isVisible={state.modal} onBackButtonPress={onBackButtonPress}>
      <GestureHandlerRootView style={{ flex: 1 }}>
        ...
        />
        {screen === 'home' && <Tags posts={data[index]} />}
